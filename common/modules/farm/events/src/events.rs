...
pub struct ClaimRewardsEvent<M: ManagedTypeApi> {
    old_farm_token: DcdtTokenPayment<M>,
    new_farm_token: DcdtTokenPayment<M>,
    farm_supply: BigUint<M>,
    reward_tokens: DcdtTokenPayment<M>,
    reward_reserve: BigUint<M>,
...
pub trait EventsModule {
    ...
    >(
        ...
        input_farming_token: DcdtTokenPayment<Self::Api>,
    ...
    >(
        ...
        output_farming_tokens: DcdtTokenPayment<Self::Api>,
        output_reward: DcdtTokenPayment<Self::Api>,
        storage_cache: StorageCache<'a, C>,
    ) {
        let epoch = self.blockchain().get_block_epoch();
        let block = self.blockchain().get_block_nonce();
    ...
    >(
        ...
        let reward_tokens = DcdtTokenPayment::new(
