    ...
    + crate::events::EventsModule
{
    ...
    fn config_enable_by_user_parameters(
        ...
        require!(
            common_token_id.is_valid_dcdt_identifier(),
            "Invalid locked token ID"
        );
        require!(
            locked_token_id.is_valid_dcdt_identifier(),
            "Invalid locked token ID"
        );
    ...
    fn add_common_tokens_for_user_pairs(&self, tokens: MultiValueEncoded<TokenIdentifier>) {
        ...
        for token in tokens {
            require!(token.is_valid_dcdt_identifier(), "Invalid token ID");
    ...
    fn set_swap_enabled_by_user(&self, pair_address: ManagedAddress) {
        ...
        let locked_token_data = self.blockchain().get_dcdt_token_data(
            &own_sc_address,
    ...
    fn get_pair_lp_token_id(&self, pair_address: &ManagedAddress) -> TokenIdentifier {
        ...
        require!(
            lp_token_id.is_valid_dcdt_identifier(),
    ...
    fn get_lp_token_value(
        ...
        let multi_value: MultiValue2<DcdtTokenPayment<Self::Api>, DcdtTokenPayment<Self::Api>> =
