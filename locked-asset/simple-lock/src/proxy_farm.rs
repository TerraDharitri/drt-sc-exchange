...
pub type EnterFarmThroughProxyResultType<M> = MultiValue2<DcdtTokenPayment<M>, DcdtTokenPayment<M>>;
pub type ExitFarmThroughProxyResultType<M> = MultiValue2<DcdtTokenPayment<M>, DcdtTokenPayment<M>>;
pub type FarmClaimRewardsThroughProxyResultType<M> =
    MultiValue2<DcdtTokenPayment<M>, DcdtTokenPayment<M>>;
pub type FarmCompoundRewardsThroughProxyResultType<M> = DcdtTokenPayment<M>;
    ...
    + dharitri_sc_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    /// `1` - FarmWithLockedRewards - rewards are META DCDT locked tokens
    #[only_owner]
    ...
    fn enter_farm_locked_token(
        ...
        let payments: ManagedVec<DcdtTokenPayment<Self::Api>> =
            self.call_value().all_dcdt_transfers().clone_value();
        require!(!payments.is_empty(), NO_PAYMENT_ERR_MSG);

        let proxy_lp_payment: DcdtTokenPayment<Self::Api> = payments.get(0);
        ...
        for p in &additional_proxy_farm_tokens {
            ...
            additional_farm_payments.push(DcdtTokenPayment::new(
    ...
    fn exit_farm_locked_token(&self) -> ExitFarmThroughProxyResultType<Self::Api> {
        ...
        let lp_proxy_token_payment = DcdtTokenPayment::new(
