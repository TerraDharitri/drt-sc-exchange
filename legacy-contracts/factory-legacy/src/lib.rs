...
pub struct OldDcdtTokenPayment<M: ManagedTypeApi> {
    pub token_type: DcdtTokenType,
    pub payment: DcdtTokenPayment<M>,
}

impl<M: ManagedTypeApi> OldDcdtTokenPayment<M> {
    pub fn new(payment: DcdtTokenPayment<M>) -> Self {
        Self {
    ...
    + dharitri_sc_modules::pause::PauseModule
{
    ...
    fn create_and_forward_custom_period(
        ...
        OldDcdtTokenPayment::new(new_token)
    }
    ...
    fn unlock_assets(&self) {
        ...
        let (token_id, token_nonce, amount) = self.call_value().single_dcdt().into_tuple();
        ...
        let mut output_locked_assets_token_amount =
            DcdtTokenPayment::new(token_id.clone(), 0, BigUint::zero());
        ...
        self.send()
            .dcdt_local_burn(&locked_token_id, token_nonce, &amount);
    ...
    fn produce_tokens_and_send(
        ...
        DcdtTokenPayment::new(token_id, sent_nonce, amount.clone())
    }
    ...
    fn set_burn_role_for_address(&self, opt_address: OptionalValue<ManagedAddress>) {
        ...
        self.send()
            ...
            .set_special_roles(
                &address,
                &self.locked_asset_token_id().get(),
                [DcdtLocalRole::NftBurn][..].iter().cloned(),
            )
            .async_call_and_exit()
    }
